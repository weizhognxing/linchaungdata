...
SET @col_exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='patients' AND COLUMN_NAME='case_integrity');
SET @sql := IF(@col_exists = 0, "ALTER TABLE patients ADD COLUMN `case_integrity` varchar(20) NOT NULL DEFAULT 'draft' AFTER `case_status`", 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

UPDATE patients
SET case_integrity = CASE WHEN case_status = 'submitted' THEN 'complete' ELSE 'draft' END
WHERE case_integrity IS NULL OR case_integrity = '' OR case_integrity = 'submitted';
